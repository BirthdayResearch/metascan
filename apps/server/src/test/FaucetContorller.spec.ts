...
describe('FaucetController (e2e)', () => {
  ...
  it('should return 200 and the expected response for valid request', async () => {
    ...
    const response = await request(app.getHttpServer()).get(`/faucet/${invalidAddress}?network=${network}`).expect(400);
  ...
  it('should return 200 and the expected response for valid request', async () => {
    ...
    const response = await request(app.getHttpServer()).get(`/faucet/${evmAddress}?network=${network}`).expect(200);
  ...
  it('should return 403 for repeated requests from the same address', async () => {
    ...
    const response = await request(app.getHttpServer()).get(`/faucet/${evmAddress}?network=${network}`).expect(403);
  ...
  it('should not cache the response for a request if the response is not successful', async () => {
    ...
    const mockCacheSet = jest.spyOn(cacheManager, 'set').mockResolvedValue(true);
    await request(app.getHttpServer()).get(`/faucet/${evmAddress}?network=${network}`).expect(500);
  ...
  it('should throttle requests from the same address within the throttle time frame', async () => {
    ...
    const response1 = await request(app.getHttpServer()).get(`/faucet/${evmAddress}?network=${network}`).expect(200);

    const response2 = await request(app.getHttpServer()).get(`/faucet/${evmAddress}?network=${network}`).expect(403);
