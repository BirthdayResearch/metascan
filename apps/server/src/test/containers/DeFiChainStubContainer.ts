...
import { Network } from '@stickyjs/testcontainers';
...
export class DeFiChainStubContainer {
  async start(): Promise<StartedDeFiChainStubContainer> {
    ...
    const defid = await new NativeChainContainer()
      ...
      .addCmd('-ethrpcport=19551')
      .addCmd('-ethrpcbind=0.0.0.0')
      .withExposedPorts(19551, 19554)
      .start();
...
export class StartedDeFiChainStubContainer {
  ...
  constructor(
    public defid: StartedNativeChainContainer,
  ...
  async getEvmURL(): Promise<string> {
    return `http://localhost:${this.defid.getMappedPort(19551)}`;
  }
}
